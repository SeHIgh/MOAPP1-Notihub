...
// KeywordWeight의 인자에 count는 왜 있는가?
...
class UserPreference {
    ...
    // Ver.3 키워드 가중치 업데이트 (키워드 횟수와 가중치 감소율 반영)
    fun updateKeywordWeights(notification: Notification) {
        val currentTime = System.currentTimeMillis()
        val decayRate = 0.1  // 가중치 감소 비율
        val maxWeight = 5.0  // 동적 상한치 (예: 5로 설정, 필요시 조정 가능)
    ...
    fun onNotificationClick(notification: Notification, readDuration: Duration) {
        ...
        for (keyword in notification.keywords) {
            ...
            // 읽은 시간이 충분히 길면 가중치 증가, 새로운 관심사로 추가
            val additionalWeight = if (readDuration.inWholeMilliseconds / 400.0 >= longReadThreshold) 0.2 else 0.1
            keywordWeight.weight = sigmoid(keywordWeight.weight + additionalWeight, maxWeight, k)
